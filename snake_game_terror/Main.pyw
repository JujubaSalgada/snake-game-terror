...
imagemIntroducao = pygame.transform.scale(pygame.image.load('wallpaperSnake.png'), (x_tela, y_tela))
chao = pygame.transform.scale(pygame.image.load('chao.png'), (x_tela, y_tela))

lista_nome_som = listdir('som')
lista_nome_imagem = listdir('imagem')
...
while loopPrincipal:
    ...
    while loopMenu:
        ...
        if analisador(31) in [31]:
            imagem_aleatoria = pygame.transform.scale(pygame.image.load(('imagem/'+lista_nome_imagem[randint(0, (len(lista_nome_imagem)-1))])), (x_tela, y_tela))
            tela.blit(imagem_aleatoria, (0,0))
    ...
    while loopGame:
        ...
        if head.colliderect(maca):
            ...
            somMastigar = pygame.mixer.Sound('som/'+lista_nome_som[randint(0, (len(lista_nome_som) - 1))])
            somMastigar.play()
            if velocidade < 13:
                velocidade += 0.2
        ...
        # imagens aleatoriamente aleatorias 
        if analisador(31) in [31, 62]:
            imagem_aleatoria = pygame.transform.scale(pygame.image.load(('imagem/'+lista_nome_imagem[randint(0, (len(lista_nome_imagem)-1))])), (x_tela, y_tela))
            tela.blit(imagem_aleatoria, (0,0))
